...
export const MemoryMatchGame = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-4 p-2 sm:p-4 rounded-lg glass-card">
        {cards.map((card, index) => (
          <div
            key={card.id}
            className="h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 rounded-lg perspective cursor-pointer"
          ...
          >
            <div className={cn(
              ...
              <div className="absolute w-full h-full flex items-center justify-center bg-primary/20 rounded-lg [backface-visibility:hidden]">
                 <Gamepad2 className="h-8 w-8 sm:h-10 sm:w-10 text-primary" />
              </div>
              {/* Back of card */}
              <div className="absolute w-full h-full flex items-center justify-center bg-secondary rounded-lg [transform:rotateY(180deg)] [backface-visibility:hidden]">
                 <card.icon className="h-8 w-8 sm:h-10 sm:w-10 text-accent" />
              </div>
            </div>
          </div>
        ))}
